...
// ── Persönlicher Dienstplan-Export (iCal / .ics) – Feature 38, #222 ──────────────
// Pro Wachgänger eine .ics mit einem VEVENT je Diensttag (Turm/Boot/HW).
// Zeiten strikt LOKAL (kein UTC/„Z"-Suffix, kein toISOString) – konsistent mit dates.js.

/** Sucht den Slot einer Person in einem Tag (assign[]). DOM-frei, testbar. */
function _findPersonStation(assign, personId){
  for(const slot of (assign || [])){
    if(slot.kind === 'tower' && (slot.occupants||[]).some(p => p && p.id === personId))
      return { name: slot.tower || 'Turm', code: slot.code || '' };
    if(slot.kind === 'boat' && (slot.occupants||[]).some(p => p && p.id === personId))
      return { name: slot.name || 'Boot', code: slot.code || '' };
    if(slot.kind === 'main'){
      const onDuty = [...(slot.mainGuards||[]), ...(slot.fuehrung||[]), ...(slot.base||[])]
        .some(p => p && p.id === personId);
      if(onDuty) return { name: 'Hauptwache', code: slot.code || 'HW' };
      // sick (außer Dienst) zählt NICHT als Diensttag → kein Event.
    }
  }
  return null;
}

/** iCal-Text-Escaping (RFC 5545): Backslash, Semikolon, Komma, Zeilenumbruch. */
...
/** Lokaler Zeitstempel YYYYMMDDTHHMMSS (floating local time, kein Z). */
function _icsLocalDT(isoDate, hour){
  const [y, m, d] = String(isoDate).split('-');
  const hh = String(Math.max(0, Math.min(23, hour|0))).padStart(2, '0');
  return `${y}${m}${d}T${hh}0000`;
}

/** Baut den .ics-Inhalt für eine Person. DOM-frei (Parameter statt Globals) → testbar.
 *  @param schedule  lastResult.schedule (Array von { assign:[...] })
 *  @param personId  Person-ID
 *  @param personName Anzeigename (für CALNAME)
 *  @param dayDates  Array ISO-Datumsstrings je Tag (computeDayDates())
 *  @param startH/endH Dienstbeginn/-ende (Stunde) */
function buildPersonICS(schedule, personId, personName, dayDates, startH, endH){
  const sH = Math.max(8, Math.min(19, startH|0));
  let eH = Math.max(8, Math.min(19, endH|0));
  if(eH <= sH) eH = sH + 1;
  const CRLF = '\r\n';
  const lines = [
    'BEGIN:VCALENDAR', 'VERSION:2.0',
    'PRODID:-//DLRG//Wachplan-Generator//DE', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
    `X-WR-CALNAME:${_icsEscape('Wachplan – ' + personName)}`,
  ];
  const stampDate = (dayDates[0] || '').replace(/-/g, '');
  (schedule || []).forEach((dayEntry, i) => {
    const iso = dayDates[i];
    if(!iso) return;
    const station = _findPersonStation(dayEntry.assign, personId);
    if(!station) return; // kein Diensttag → kein Event
    const uid = `wachplan-${personId}-${iso}@dlrg`;
    lines.push(
      'BEGIN:VEVENT',
      `UID:${uid}`,
      `DTSTAMP:${(stampDate||iso.replace(/-/g,''))}T000000`,
      `DTSTART:${_icsLocalDT(iso, sH)}`,
      `DTEND:${_icsLocalDT(iso, eH)}`,
      `SUMMARY:${_icsEscape(station.name)}`,
    );
    if(station.code) lines.push(`LOCATION:${_icsEscape(station.code)}`);
...
/** Export-Wrapper: liest Globals, baut .ics, lädt sie herunter. */
function exportPersonICS(personId){
  if(!lastResult?.schedule){ showToast('Erst einen Plan generieren'); return; }
  const person = people.find(p => p.id === personId);
  if(!person){ showToast('Person nicht gefunden'); return; }
  const dayDates = computeDayDates();
  const ics = buildPersonICS(lastResult.schedule, personId, person.name, dayDates,
    serviceStartHour, serviceEndHour);
  const safeName = (person.name || 'person').replace(/[^\wäöüÄÖÜß-]+/g, '_');
  const blob = new Blob([ics], { type:'text/calendar;charset=utf-8' });
  downloadBlob(blob, `wachplan-${safeName}.ics`);
}
