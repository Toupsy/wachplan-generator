...
(function () {
  ...
  // Öffnet die aufklappbare Ausgabe-Sektion (Feature: aufklappbare Sektionen) und scrollt dorthin.
  // Fällt auf reines Scrollen zurück, falls openOutputSection (render-output.js) (noch) nicht da ist.
  function gotoOutputSection(key) {
    if (typeof openOutputSection === 'function') {
      openOutputSection(key);
    } else {
    ...
    hamburgerMenu.querySelectorAll('.ham-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        if (action === 'info') {
          ...
          switchMobilePanel(1);
          setTimeout(() => gotoOutputSection('boatStats'), 80);
        } else if (action === 'matrix') {
          switchMobilePanel(1);
          setTimeout(() => gotoOutputSection('matrix'), 80);
        }
      });
    });
  }
