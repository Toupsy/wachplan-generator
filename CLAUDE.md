...
```js
people[]           // { id, name, role:'F'|'B'|'W', experienced:boolean, labels:'', enableLabels:boolean }
                   // F = Führung (experienced irrelevant)
                   // B = Bootsführer (experienced: true=erfahren, false=unerfahren)
                   // W = Wachgänger (experienced: true=erfahren, false=unerfahren)
...
**Rollen:** F = Führung, B = Bootsführer, W = Wachgänger  
**MAIN_ID = 0** (Pseudo-ID der Hauptwache)  
**Erfahrung:** Via `experienced:boolean` für B und W (für F irrelevant)
...
### Feature 13: Vereinfachtes Rollenmodell mit Erfahrungs-Flag
Ersetzt vorheriges E/U/BF-E/BF-U Modell durch einheitliches System:
- **Rollen:** F (Führung) | B (Bootsführer) | W (Wachgänger)
- **Erfahrung:** `experienced:boolean` für B und W (für F irrelevant)
- **Effektives Level:** `effLevel(p)` Hilfsfunktion → F/E/U für Algorithmus
- **Turm-Pairing:** `effLevel()` in `bestPair()` → experienced wie 'E', unerfahren wie 'U'; verhindert zwei Unerfahrene zusammen auf Turm (UU-Penalty)
- **Boot-Rotation:** `experienced` hat **keine** Auswirkung (faire Rotation für alle BF)
- **Rückwärtskompatibilität:** `migratePerson()` in `state-io.js` (STATE_VERSION 4→5) konvertiert alte E/U/bfLevel-Daten automatisch
- **UI:** Dropdown mit 3 Optionen (F/B/W) + Checkbox „Erfahren" (sichtbar bei B/W, ausgeblendet bei F)
- **Export:** `roleLabel(p)` liefert korrekte Beschriftung in CSV/XLSX mit Erfahrungsdetail
